...
class flow(object):
    ...
    def track(self, image):
        ...
        self.old_img = image2
        npflowx = np.array(flow2[..., 0])[int(self.position[1]-top):int(self.position[1]-top + self.size[1]), int(self.position[0]-left):int(self.position[0]-left + self.size[0])]
        npflowy = np.array(flow2[..., 1])[int(self.position[1]-top):int(self.position[1]-top + self.size[1]), int(self.position[0]-left):int(self.position[0]-left + self.size[0])]
        ...
        sumy = 0
        while(i < 1):
            meanx = np.median(npflowx)
            meany = np.median(npflowy)
            #print(meany)
            meanx = np.nan_to_num(meanx)
            meany = np.nan_to_num(meany)
            sumx += meanx
            sumy += meany
            npflowx -= meanx
            npflowy -= meany
